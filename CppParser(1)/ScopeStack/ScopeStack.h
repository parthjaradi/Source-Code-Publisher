...
/*
  ...
  Maintenance History:
  ====================
  ver 2.3 : 14 March 2019
  - added an method to return the required data while parsing the file
  ver 2.2 : 29 Oct 2016
...
namespace CodeAnalysis
{
  ...
  // -----< modified version of TreeWalk that returns required data >----------
  template<typename element>
  std::set<std::string> newTreeWalk(element* pItem, bool details = false)
  {
	  std::set<std::string> rset;


	  // getting  include files data for every file
	  auto iter2 = pItem->statements_.begin();
	  while (iter2 != pItem->statements_.end())
	  {
		  std::string includeStmt = (*iter2)->ToString();
		  if (includeStmt.find("<") == std::string::npos && includeStmt.find("../..") == std::string::npos)
			  rset.insert(includeStmt);
		  ++iter2;
	  }
	  ...
	  // getting  Starting and Ending  Positions of every block
	  std::string divPositions = "Div"+pItem->type_ + ":" + std::to_string(pItem->startLineCount_) + ":" + std::to_string(pItem->endLineCount_);
	  
	  rset.insert(divPositions);

	  while (iter != pItem->children_.end())
	  {
		  std::set<std::string> tempSet;
		  tempSet = newTreeWalk(*iter);
